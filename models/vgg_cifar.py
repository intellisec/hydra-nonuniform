...
class VGG(nn.Module):
    def __init__(self, features, last_conv_channels_len, linear_layer, num_classes=10, prune_reg='weight', task_mode='prune'):
...
def make_layers(cfg, conv_layer, batch_norm=True, num_classes=10, prune_reg='weight', task_mode='prune'):
    layers = []
    in_channels = 3
    for v in cfg:
        if v == "M":
